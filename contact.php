...
$recipient = 'schmidts.erben@gmail.com';
...
$serverName = 'hausprojekt-schoeneiche.github.io';
...
<input
	...
	Ihr echter Name (<i>damit unsere Antwort Sie auch erreicht.</i>)
</p>
<p>
<input
	name="subject"
...
<p>
<i>Bitte geben Sie hier den Text ihrer Nachricht ein.</i>
</p>
<textarea
	name="body"
	rows="10"
...
function sendMail()
{
	...
	if (!preg_match("/^[\w\+\-\.\~]+\@[\-\w\.\!]+$/", $email)) {
		$messages[] = "Das ist keine gültige Email-Adresse. Vielleicht haben Sie den @x.com-Teil vergessen?";
	}
	$realName = $_POST['realname'];
	if (!preg_match("/^[\w\ \+\-\'\"]+$/", $realName)) {
		$messages[] = "Das Namensfeld darf nur Buchstaben, Zahlen, Leerzeichen und +/-Zeichen enthalten. Wir entschuldigen uns für entstandene Unannehmlichkeiten";
	}
	...
	if (preg_match('/^\s*$/', $subject)) {
		$messages[] = "Bitte geben Sie einen Betreff an.";
	}

	$body = $_POST['body'];
	# Make sure the message has a body
        if (preg_match('/^\s*$/', $body)) {
		$messages[] = "Ihre Nachricht war leer. Wollten Sie uns etwas mitteilen? Klicken sie auf die Schaltfläche Abbrechen falls sie uns keine Nachricht schicken wollten.";
	}
	...
	mail($recipient,
		...
		$body,
		"Von: $realName <$email>\r\n" .
		"Antwort schicken an: $realName <$email>\r\n");
...
<h1>Vielen Dank</h1>
<p>
Danke für die Nachricht! Wir werden ihnen in Kürze antworten.
</p>
<form method="POST" action="<?php echo $_SERVER['DOCUMENT_URL']?>">
<input type="submit" name="continue" value="Hier klicken um weiterzugehen"/>
<input
	type="hidden"
	name="returnurl"
	value="<?php echo $escapedReturnUrl?>"/>
